...
智能体在调用外部工具完成任务时，参数会被传给应用、写入数据库或保存在服务日志中。一个以往被忽视的问题是：智能体可能在完成任务的同时，通过工具参数披露当前操作并不需要的敏感信息。取消订单需要订单编号，却不需要用户的病史；发送通知需要正文，却不需要账户余额。这些字段在工具模式中是可选的，服务端也能正常接收，但披露行为已经超出了当前操作的目的。
