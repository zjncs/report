...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \caption{Adapter 边界。上游文件先被解析为保留来源语义的 \code{SourceCase}，通过版本锁定后才进入公共构造流程；隐私规则不进入 Adapter。}
...
代码中的 \code{SourceCase}、\code{SourceTrace}、\code{NormalizedCall} 和 \code{NormalizedTool} 分别承载这些信息。表\ref{tab:adapter-interface}列出公共 Builder 实际依赖的字段。这里的“统一”仅指访问接口一致，不把不同来源的参考证据解释成相同强度的 Gold。
